...
/datum/game_mode/colonialmarines/proc/lore_announcement_lv733()
	...
	addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(lv733_announce_arakawa), "Алмаер, говорит Капитан Аракава, ДК ИТМ «Фого». Гарнизон «Блэкбуш» подтверждён на тренировочной ЛЗ «Дельта». Первый рейс уходит через восемь минут.\n\nВсем наземным подразделениям — удержать текущие позиции на время эвакуации.", "ДК ИТМ «Фого», Капитан Кэндзи Аракава", 'sound/AI/commandreport.ogg'), 50 MINUTES)
	// Тест: крушение должно происходить на 5-й минуте. У события есть свой SHIP_CRASH_WARN_DELAY (2 минуты
	// между announce() и start(), см. ship_crash.dm) - поэтому триггерим на 3-й минуте, удар придётся на 5-ю.
	addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(marine_announcement), "МЭЙДЭЙ. Говорит транспорт «Гера», борт два-два. Поражены при наборе высоты. Двигатели отказали.\n\nПадаем в сторону городского сектора— &^@%###--- —на борту— &^@%$sss----", "Транспорт ИТМ «Гера», борт 2-2", 'sound/AI/commandreport.ogg'), 3 MINUTES)
	addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(trigger_lv733_ship_crash)), 3 MINUTES)
	addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(xeno_announcement), "Вы чувствуете это, дети мои? Один из летающих ульев хозяев упал с неба прямо в городские владения.\n\nТам могут быть выжившие", "everything", QUEEN_MOTHER_ANNOUNCE), 57 MINUTES)
	addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(lv733_announce_arakawa), "Всем подразделениям, говорит «Фого». Борт два-два потерян. Место крушения — городской сектор, ваш район операций.\n\nРесурсов на немедленную поисково-спасательную операцию нет. Выжившие с борта предоставлены сами себе до прихода подкрепления.", "ДК ИТМ «Фого», Капитан Кэндзи Аракава", 'sound/AI/commandreport.ogg'), 58 MINUTES)
